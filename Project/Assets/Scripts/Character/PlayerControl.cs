...
public class PlayerControl : MonoBehaviour
{
    ...
    public static float MOVE_SPEED = 7.0f; // 이동 속도. 5.0->7.0
    public static float SLOW_MOVESPEED = 0.7f; // 이동 속도 감소 퍼센테이지
    ...
    public float step_timer = 0.0f; // 스탭 타이머.
    public float DOTTimer = 0f; // 도트 대미지 타이머
   // Use this for initialization
    ...
    private void move_control()
    {
        ...
        if (this.gameObject.GetComponent<DeburfControl>().GetDeburfActivate())  // 회전 계산을 위해 디버프를 임시 해제
        {
        ...
        if (move_vector.magnitude > 0.01f)
        {
            ...
            Quaternion.Lerp(this.transform.rotation, q, 0.2f);  // Lerp 비율을 0.1->0.2로
        }
    ...
    void OnGUI()
    {
        ...
        else
        {
            ...
            {
                GUI.Label(new Rect(x, y, 200.0f, 20.0f), "X:줍는다", guistyle);
            }
        }
        ...
        switch (this.step)
        {
            case STEP.EATING:
                ...
                break;
            case STEP.REPAIRING:
                GUI.Label(new Rect(x + 200.0f, y, 200.0f, 40.0f), "수리중..",
                guistyle);
                break;
        }
    ...
    void Update()
    {
        ...
        float eat_time = 0.5f; // 사과는 2초->0.5초에 걸쳐 먹는다.
        
        float repair_time = 0.5f; // 수리에 걸리는 시간도 2초->0.5초.

        float attack_time = 0.2f;   // 공격 시간 0.2초

        float DOTTime = 1.0f;       // 도트 대미지 간격
